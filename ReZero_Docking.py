...
def none_or_str(value):
    ...
    return value
# Inference mode
parser.add_argument('--mode', type=str, default='virtual_screen', help='Inferece mode, [crossdock, apodock, virtual_screen]')
...
start_ligand_id, end_ligand_id = None, None
    ...
    for index, orig_complex_graph in enumerate(orig_list):
        ...
        if not args.no_chi_angle and args.mode != 'virtual_screen':
            ...
            for rank, pos in enumerate(protein_atom_pos):
                ...
                ligand_description = os.path.join(os.path.dirname(protein_path), holo_ligand_name+'.sdf') # complex_name[0] is the protein name
                mod_prot = ModifiedPDB(pdb_path=protein_path, ligand_description=ligand_description, pocket_pos=pos)
                # if args.mode == 'apo_docking':
                #     if rank == 0: # additionally save a copy named rank1
                #         pickle.dump(pos, open(os.path.join(write_dir, f'rank{rank+1}_pocket_coords.pkl'), 'wb')) # save the predicted pocket comformation for calculating metrics later
                ...
                #     mod_prot.to_pdb(os.path.join(write_dir, f'rank{rank+1}_protein.pdb'), pocket_only=True)
                # pdb.set_trace()
                mod_prot.to_pdb(os.path.join(write_dir, f'rank{rank+1}_pocket.pdb'), pocket_only=True)
    # pdb.set_trace()
