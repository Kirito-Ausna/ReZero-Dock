...
parser.add_argument('--mode', type=str, default='crossdock', help='Inferece mode, crossdock, redocking or apodock')
...
for idx, orig_complex_graphs in tqdm(enumerate(test_loader), desc="Generating Docking Conformation", total=len(test_dataset)): 
    # try:
    confidence_data_list = None
    data_list = []
    orig_list = []
    for orig_complex_graph in orig_complex_graphs:
    ...
    for index, orig_complex_graph in enumerate(orig_list):
        ...
        data_list = data_list[index*N:(index+1)*N]
        confidence = confidence[index*N:(index+1)*N] if confidence is not None else None

        ligand_pos_list = [complex_graph['ligand'].pos.cpu().numpy() + orig_complex_graph.original_center.cpu().numpy() for complex_graph in data_list]
        ligand_pos = np.asarray(ligand_pos_list)
        if not args.no_chi_angle and args.mode != 'virtual_screening':
            protein_atom_pos_list = [complex_graph['atom'].pos.cpu().numpy() + orig_complex_graph.original_center.cpu().numpy() for complex_graph in data_list]
